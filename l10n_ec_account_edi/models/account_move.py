...
class AccountMove(models.Model):
    ...
    def _l10n_ec_get_taxes_grouped_by_tax_group(self, exclude_withholding=True):
        ...
        def filter_withholding_taxes(base_line, tax_values):
            ...
            repartition_line = tax_values.get("tax_repartition_line")
            if (
                not repartition_line
                or not repartition_line.tax_id
                or not repartition_line.tax_id.tax_group_id
            ):
                return True  # Si no hay línea de reparto, no se considera retención

            return repartition_line.tax_id.tax_group_id.id not in withhold_group_ids
