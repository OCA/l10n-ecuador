...
class AccountEdiDocument(models.Model):
    ...
    def _l10n_ec_validate_tax_dict(self, tax_data):
        """
        Valida que el dict tax_data contenga un objeto válido de account.tax.
        Útil para tests previos antes de generar el XML.
        """
        try:
            if tax_data.get("tax") and tax_data["tax"]._name == "account.tax":
                return True
            if "taxes_data" in tax_data:
                for entry in tax_data["taxes_data"]:
                    if entry.get("tax") and entry["tax"]._name == "account.tax":
                        return True
            if "base_line_x_taxes_data" in tax_data:
                nested = tax_data["base_line_x_taxes_data"][0][1][0]
                if nested.get("tax") and nested["tax"]._name == "account.tax":
                    return True
        except Exception:
            return False
        return False
    ...
    def _l10n_ec_prepare_tax_vals_edi(self, tax_data):
        ...
        Extrae el impuesto desde 'tax', 'taxes_data' o desde tax_details anidados.
        ...
        # Segundo intento: buscar en taxes_data
        ...
        # Tercer intento: buscar en base_line_x_taxes_data → tax_details → taxes_data
            ...
            try:
                ...
                source = "anidado en base_line_x_taxes_data → tax_details"
            except Exception:
        ...
        # Validación de tipo sin usar isinstance()
        try:
