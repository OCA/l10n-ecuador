...
class TestL10nECEdiCommon(AccountEdiTestCommon, TestL10nECCommon):
    ...
    def setUpClass(cls):
        ...
        # Cargar formato EDI y plan contable ecuatoriano manualmente
        try:
            edi_format = cls.env.ref("l10n_ec_account_edi.edi_format_ec_sri", raise_if_not_found=False)
            chart_template = cls.env.ref("l10n_ec.account_chart_template", raise_if_not_found=False)
            if chart_template:
                chart_template.try_loading()
        except Exception as e:
            cls.env.cr.rollback()
            print(f"[WARN] Error cargando datos base de EDI Ecuador: {e}")

        # Crear archivo XML básico
        ...
        file_path = os.path.join("l10n_ec_account_edi", "tests", "certificates", "test.p12")
        file_content = misc.file_open(file_path, mode="rb").read()

        # Crear certificado de firma electrónica válido
        cls.certificate = cls.env["sri.key.type"].sudo().create({
